...
class QueryParamsValidatorSpec extends HmrcSpec with ApplicationWithCollaboratorsFixtures with EitherValues with FixedClock
  ...
  "parseParams" when {
    ...
    val WeDontCareAboutTheName = ParamName.AccessType

    "expecteds" should {
      "no-value-expected passes" in {
        QueryParamsValidator.NoValueExpected(WeDontCareAboutTheName)(Seq()) shouldBe ().validNel
      }

      "no-value-expected fails" in {
        shouldFail(QueryParamsValidator.NoValueExpected(WeDontCareAboutTheName)(Seq("1")))
      }

      "at least-one-value-expected passes" in {
        QueryParamsValidator.AtLeastOneValue(WeDontCareAboutTheName)(Seq("1")) shouldBe List("1").validNel
      }
      "at least-one-value-expected passes with several values" in {
        QueryParamsValidator.AtLeastOneValue(WeDontCareAboutTheName)(Seq("1", "2", "3")) shouldBe List("1", "2", "3").validNel
      }
      "at least-one-value-expected fails with no values" in {
        shouldFail(QueryParamsValidator.AtLeastOneValue(WeDontCareAboutTheName)(Seq()))
      }

      "single-value-expected passes" in {
        QueryParamsValidator.SingleValueExpected(WeDontCareAboutTheName)(Seq("1")) shouldBe "1".validNel
      }
      "single-value-expected fails" in {
        shouldFail(QueryParamsValidator.SingleValueExpected(WeDontCareAboutTheName)(Seq("1", "2")))
        shouldFail(QueryParamsValidator.SingleValueExpected(WeDontCareAboutTheName)(Seq()))
      }

      "boolean-expected passes" in {
        QueryParamsValidator.BooleanValueExpected(WeDontCareAboutTheName)("true") shouldBe true.validNel
        QueryParamsValidator.BooleanValueExpected(WeDontCareAboutTheName)("false") shouldBe false.validNel
      }
      "boolean-expected fails" in {
        shouldFail(QueryParamsValidator.BooleanValueExpected(WeDontCareAboutTheName)("not-a-boolean"))
      }

      "integer-expected passes" in {
        QueryParamsValidator.IntValueExpected(WeDontCareAboutTheName)("1") shouldBe 1.validNel
        QueryParamsValidator.IntValueExpected(WeDontCareAboutTheName)("4") shouldBe 4.validNel
        QueryParamsValidator.IntValueExpected(WeDontCareAboutTheName)("-5") shouldBe -5.validNel
      }
      "integer-expected fails" in {
        shouldFail(QueryParamsValidator.IntValueExpected(WeDontCareAboutTheName)("not-an-int"))
        shouldFail(QueryParamsValidator.IntValueExpected(WeDontCareAboutTheName)("1.5"))
      }

      "positive-integer-expected passes" in {
        QueryParamsValidator.IntValueExpected(WeDontCareAboutTheName)("1") shouldBe 1.validNel
        QueryParamsValidator.IntValueExpected(WeDontCareAboutTheName)("4") shouldBe 4.validNel
        QueryParamsValidator.IntValueExpected(WeDontCareAboutTheName)("0") shouldBe 0.validNel
      }
      "positive-integer-expected fails" in {
        shouldFail(QueryParamsValidator.PositiveIntValueExpected(WeDontCareAboutTheName)("-5"))
        shouldFail(QueryParamsValidator.PositiveIntValueExpected(WeDontCareAboutTheName)("not-an-int"))
        shouldFail(QueryParamsValidator.PositiveIntValueExpected(WeDontCareAboutTheName)("1.5"))
      }

      "instant-value-expected passes" in {
        QueryParamsValidator.InstantValueExpected(WeDontCareAboutTheName)(nowAsText) shouldBe instant.validNel
      }
      "instant-value-expected fails" in {
        shouldFail(QueryParamsValidator.InstantValueExpected(WeDontCareAboutTheName)("not-an-instant"))
      }
    }
    ...
    "extract" should {
      ...
      "extract valid params - status filter" in {
        ...
        test(Map("status" -> Seq("PENDING_RESPONSIBLE_INDIVIDUAL_VERIFICATION"))) shouldBe List(
          MatchOneStateQP(State.PendingResponsibleIndividualVerification)
        ).validNel
        test(Map("status" -> Seq("PENDING_GATEKEEPER_CHECK"))) shouldBe List(MatchOneStateQP(State.PendingGatekeeperApproval)).validNel
        test(Map("status" -> Seq("PENDING_SUBMITTER_VERIFICATION"))) shouldBe List(MatchOneStateQP(State.PendingRequesterVerification)).validNel
