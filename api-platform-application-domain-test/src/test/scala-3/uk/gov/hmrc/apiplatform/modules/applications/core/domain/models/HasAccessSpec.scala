  ...
  "hasAccess" when {
    "Standard Access" should {
      ...
      "privacyPolicyLocation should return value when set" in {
        objInTest.privacyPolicyLocation shouldBe standardAccessOne.privacyPolicyUrl.map(PrivacyPolicyLocation.Url(_))
      }
      "termsAndConditionsLocation should return value when set" in {
        objInTest.termsAndConditionsLocation shouldBe standardAccessOne.termsAndConditionsUrl.map(TermsAndConditionsLocation.Url(_))
      }
