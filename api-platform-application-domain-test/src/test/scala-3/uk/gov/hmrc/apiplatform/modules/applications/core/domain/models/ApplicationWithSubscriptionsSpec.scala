...
class ApplicationWithSubscriptionsSpec extends BaseJsonFormattersSpec with ApplicationWithCollaboratorsFixtures {
  import ApplicationWithSubscriptionsSpec.*
  ...
  "ApplicationWithSubscriptions" should {
    ...
    "support withToken to replace it" in {
      val newApp = NoSubs.example.withToken(ApplicationTokenData.two)

      newApp.details.token shouldBe ApplicationTokenData.two
    }

    "support modifyToken" in {
      val app    = NoSubs.example.withToken(ApplicationTokenData.one)
