...
class ApplicationWithCollaboratorsSpec extends BaseJsonFormattersSpec with ApplicationWithCollaboratorsFixtures {
  import ApplicationWithCollaboratorsSpec.*
  ...
  "ApplicationWithCollaborators" should {
    ...
    "identify a role" in {
      val app = example.copy(collaborators = Set(Dev.example, Admin.example))
      app.roleFor(Dev.example.userId).value shouldBe Collaborator.Role.Developer
      app.roleFor(Admin.example.userId).value shouldBe Collaborator.Role.Administrator
    }

    "identify a role by email" in {
      val app = example.copy(collaborators = Set(Dev.example, Admin.example))
      app.roleFor(Dev.example.emailAddress).value shouldBe Collaborator.Role.Developer
      app.roleFor(Admin.example.emailAddress).value shouldBe Collaborator.Role.Administrator
