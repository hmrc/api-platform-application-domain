...
class SubmissionIdSpec extends BaseJsonFormattersSpec {
  ...
  "SubmissionId" should {
    "apply raw text" in {
      ...
      SubmissionId.apply(in.toString()) shouldBe Some(in)
    }
    ...
    "unsafeApply text" in {
      ...
      SubmissionId.unsafeApply(in.toString()) shouldBe in
    }
    ...
    "convert to json" in {
      Json.toJson(aSubmissionId) shouldBe JsString(aSubmissionId.toString())
    }
