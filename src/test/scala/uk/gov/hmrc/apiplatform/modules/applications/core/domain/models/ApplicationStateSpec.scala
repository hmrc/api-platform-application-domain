  ...
  "ApplicationState" should {
    ...
    "transition to DELETED from any state" in {
      for (state <- State.values) {
        ApplicationState(state, updatedOn = now()).toDeleted(now())
      }
    }

    "transition to TESTING from any state" in {
      for (state <- State.values) {
        ApplicationState(state, updatedOn = now()).toTesting(now())
      }
    }

    "transition to PRODUCTION" in {
      ApplicationState(PRE_PRODUCTION, updatedOn = now()).toProduction(now())
    }

    "fail transistion to PRODUCTION" in {
      for (state <- State.values.filterNot(_ == State.PRE_PRODUCTION)) {
        intercept[RuntimeException] {
          ApplicationState(state, updatedOn = now()).toProduction(now())
        }
      }
    }

    "transition to PRE_PRODUCTION" in {
      ApplicationState(PENDING_REQUESTER_VERIFICATION, updatedOn = now()).toPreProduction(now())
    }

    "fail transistion to PRE_PRODUCTION" in {
      for (state <- State.values.filterNot(_ == State.PENDING_REQUESTER_VERIFICATION)) {
        intercept[RuntimeException] {
          ApplicationState(state, updatedOn = now()).toPreProduction(now())
        }
      }
    }

    "transition to PENDING_REQUESTER_VERIFICATION" in {
      ApplicationState(PENDING_GATEKEEPER_APPROVAL, updatedOn = now()).toPendingRequesterVerification(now())
    }
    ...
    "fail transistion to PENDING_REQUESTER_VERIFICATION" in {
      for (state <- State.values.filterNot(_ == State.PENDING_GATEKEEPER_APPROVAL)) {
        intercept[RuntimeException] {
          ApplicationState(state, updatedOn = now()).toPendingRequesterVerification(now())
        }
      }
    }

    "transition to PENDING_RESPONSIBLE_INDIVIDUAL_VERIFICATION" in {
      ApplicationState(TESTING, updatedOn = now()).toPendingResponsibleIndividualVerification(email.text, "Fred Flintstone", now())
    }

    "fail transistion to PENDING_RESPONSIBLE_INDIVIDUAL_VERIFICATION" in {
      for (state <- State.values.filterNot(_ == State.TESTING)) {
        intercept[RuntimeException] {
          ApplicationState(state, updatedOn = now()).toPendingResponsibleIndividualVerification(email.text, "Fred Flintstone", now())
        }
      }
    }

    "transition to PENDING_GATEKEEPER_APPROVAL" in {
      ApplicationState(TESTING, updatedOn = now()).toPendingGatekeeperApproval(email.text, "Fred Flintstone", now())
    }

    "fail transistion to PENDING_GATEKEEPER_APPROVAL" in {
      for (state <- State.values.filterNot(_ == State.TESTING)) {
        intercept[RuntimeException] {
          ApplicationState(state, updatedOn = now()).toPendingGatekeeperApproval(email.text, "Fred Flintstone", now())
        }
      }
    }
