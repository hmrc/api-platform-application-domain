...
class MarkSpec extends utils.HmrcSpec {
  ...
  "Monoid of mark" should {
    ...
    "work for single value of warn" in {
      markMonoid.combineAll(List[Mark](Warn)) shouldBe Warn
    }
    "work for single value of fail" in {
      markMonoid.combineAll(List[Mark](Fail)) shouldBe Fail
    }
